...
public class offMesh : MonoBehaviour
{
        ...
        while (true)
        {
            if (agent.isOnOffMeshLink)
            {
                ...
                if (method == OffMeshLinkMoveMethod.Parabola)
                    ...
                    yield return StartCoroutine(Arrampica(agent, 0.1f,5f));
    ...
    IEnumerator Arrampica(NavMeshAgent agent, float height, float duration)
    {
        OffMeshLinkData data = agent.currentOffMeshLinkData;
        Vector3 startPos = agent.transform.position;
        Vector3 endPos = data.endPos + Vector3.up * agent.baseOffset;
        ...
        {
            // float speed = 2f;
            agent.GetComponent<Animator>().SetBool("Arrampica", true);
            float yOffset = height * 4.0f * (normalizedTime - normalizedTime * normalizedTime);
            agent.transform.position = Vector3.Lerp(startPos, endPos, normalizedTime) + yOffset * Vector3.up;
            normalizedTime += Time.deltaTime / duration;
            yield return null;
